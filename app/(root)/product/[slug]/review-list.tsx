...
import { ReviewInputSchema } from '@/lib/validation/validator'
...
export default function ReviewList({
  ...
  const { ref, inView } = useInView({ triggerOnce: true }) //we can use inView to fetch Reviews from backend
  const reload = async () => {
    try {
      const res = await getReviews({ productId: product._id, page: 1 }) //call getReviews server actions
      setReviews([...res.data])   //update reviews by setReviews
  ...
  const onSubmit: SubmitHandler<CustomerReview> = async (values) => {
    const res = await createUpdateReview({
      ...
      path: `/product/${product.slug}`, //current path
    })
    if (!res.success)
      return toast({
        variant: 'destructive',
